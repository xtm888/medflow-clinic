...
/**
 * Shared StockReceiver Component
 * Consolidates duplicate stock receiving logic from Frame, OpticalLens, ContactLens, Reagent inventories
 *
 * @param {Object} item - The inventory item (frame, lens, etc.)
 * @param {Function} service - The inventory service with addStock method
...
const StockReceiver = ({
  ...
  const {
    ...
    itemSubtitle = '',
    unitLabel = 'unites',
    currency = item?.pricing?.currency || 'CDF',
    costPrice = item?.pricing?.costPrice || item?.pricing?.unitCost || 0,
    currentStock = item?.inventory?.currentStock || item?.inventory?.available || 0,
    showWarranty = false,
    showExpiration = false,
    showPurchaseOrder = true,
    supplierFields = ['name', 'contact']
  } = config;
  ...
  // Handle form change with nested object support
    ...
    try {
      ...
      const data = {
        ...
        supplier: formData.supplier.name ? {
          ...
          reference: formData.supplier.reference || undefined,
          invoiceNumber: formData.supplier.invoiceNumber || undefined
        } : undefined,
        cost: {
          unitCost: parseFloat(formData.unitCost) || 0,
          totalCost: (parseFloat(formData.unitCost) || 0) * quantity,
          currency: formData.currency
        },
        unitCost: parseFloat(formData.unitCost) || 0,
        warrantyExpiry: showWarranty && formData.warrantyExpiry ? formData.warrantyExpiry : undefined,
        expirationDate: showExpiration && formData.expirationDate ? formData.expirationDate : undefined,
        notes: formData.notes || undefined
      };

      await service.addStock(item._id || item.id, data);
  ...
  const newStock = currentStock + (parseInt(formData.quantity) || 0);
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className={`grid grid-cols-${Math.min(supplierFields.length, 3)} gap-2`}>
              {supplierFields.includes('name') && (
                <input
                  type="text"
                  name="supplier.name"
              ...
              {supplierFields.includes('contact') && (
                <input
                  type="text"
              ...
              {supplierFields.includes('reference') && (
                <input
                  type="text"
              ...
              {supplierFields.includes('invoiceNumber') && (
                <input
                  type="text"
          ...
          <div>
            ...
            {totalCost > 0 && (
              <p className="mt-1 text-sm text-gray-500">
                Cout total: {formatCurrency(totalCost, formData.currency)}
              </p>
            )}
          </div>

          {/* Warranty (for frames, etc.) */}
          {showWarranty && (
            <div>
          ...
          {/* Expiration (for lenses, reagents, etc.) */}
          {showExpiration && (
            <div>
          ...
          <div className="bg-gray-50 rounded-lg p-3">
            <p className="text-sm text-gray-600">
              Stock actuel: <strong>{currentStock}</strong> {unitLabel}
            </p>
            <p className="text-sm text-gray-600">
              Apres reception: <strong className="text-green-600">{newStock}</strong> {unitLabel}
            </p>
          </div>
