...
async function checkProviderConflicts(providerId, startTime, endTime, excludeAppointmentId = null) {
  ...
  if (conflictingAppointments.length > 0) {
    return {
      ...
      message: `Provider has ${conflictingAppointments.length} conflicting appointment(s) during this time`,
...
async function checkBufferTimeViolations(providerId, startTime, endTime, appointmentType, excludeAppointmentId = null) {
    ...
    if (aptEnd <= startTime) {
      ...
      if (actualGap < requiredBuffer) {
        violations.push({
          ...
          message: `Only ${Math.round(actualGap)} minutes gap before this appointment (need ${requiredBuffer} minutes)`
        });
      }
    }
    ...
    if (aptStart >= endTime) {
      ...
      if (actualGap < requiredBuffer) {
        violations.push({
          ...
          requiredBuffer: requiredBuffer,
          actualGap: Math.round(actualGap),
          message: `Only ${Math.round(actualGap)} minutes gap after this appointment (need ${requiredBuffer} minutes)`
        });
      }
    }
  }
  ...
  if (violations.length > 0) {
    return {
      hasViolation: true,
      violations,
      message: `Buffer time violations detected: ${violations.map(v => v.message).join('; ')}`
    };
  }
